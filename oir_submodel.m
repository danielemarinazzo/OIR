%% OIR - derivation of a submodel of an ISS model

% A,C,K,V - parameters of the ISS model to be reduced
% ired - indexes of the processes to remove

function [CR,KR,VR]=oir_submodel(A,C,K,V,ired)

KVSQRT = K*chol(V,'lower');

CR=C(ired,:);

[KR,VR,rep] = ss2iss(A,C(ired,:),KVSQRT*KVSQRT',V(ired,ired),K*V(:,ired)); % reduced model innovations covariance
