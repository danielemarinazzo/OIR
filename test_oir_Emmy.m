...
%% load data
load('test_ROI_epilepsy.mat'); % DM change here
%% parameters
Fs=200; % sampling frequency % DM change here
nfft=1000;
%% OIR analysis
nROIs=size(COND_1,2); % change here for the number of ROIs, here they are supposed to be the same for the two conditions,
% but you can analyze only one condition at time
Mv=ones(1,nROIs); %number of series in each block % DM change here
momax=2;% maximum model order
M=length(Mv);
allN=cell(M,1);
dO_r=cell(M,1); %deltaOIR - COND_1
dOf_r=cell(M,1); %spectral deltaOIR - COND_1
dO_a=cell(M,1); %deltaOIR -COND_2tesia
dOf_a=cell(M,1); %spectral deltaOIR - COND_2tesia

for N=3:M
    
    comb=nchoosek(1:M,N); % all multiplets of size N
    allN{N}=comb;
    DO_COND_1=cell(size(comb)); DOF_COND_1=cell(size(comb));
    DO_COND_2=cell(size(comb)); DOF_COND_2=cell(size(comb));
    ...
    for n=1:size(comb,1)
        ...
        for k=1:size(comb,2) % vary target inside the multiplet
            ...
            [~,pottmdl_r] = oir_mosVAR(COND_1',momax);
            [~,pottmdl_a] = oir_mosVAR(COND_2',momax);
            %%% VAR identification procedure
            [Am_r,Su_r]=idMVAR(COND_1',pottmdl_r,0);
            [Am_a,Su_a]=idMVAR(COND_2',pottmdl_a,0);
            ...
            DOF_COND_1{n,k} = DOF_r;
            DO_COND_1{n,k} = DO_r;
            DOF_COND_2{n,k} = DOF_a;
            DO_COND_2{n,k} = DO_a; 
        end
    end
    dO_r{N-1}=DO_COND_1;
    dOf_r{N-1}=DOF_COND_1;
    dO_a{N-1}=DO_COND_2;
    dOf_a{N-1}=DOF_COND_2;
end
